...
import com.beyond.bpush.core.entity.PayloadHistory;
...
public class PayloadServiceImpl extends BaseService implements PayloadService {
    ...
    public static final String SQL_PAYLOAD_CLIENT_ALL_INSERT = "insert into payload_client(payloadId, userId, productId, statusId, createTime, onlineMode, errorId, errorMsg)values(?, ?, ?, ?, ?, ?, ?, ? )";
    ...
    public void postSaveAfterSent(final Payload payload) {
        ...
        if (payload.getClients() != null){
            ...
            try {
                mainJdbc.batchUpdate(SQL_PAYLOAD_CLIENT_ALL_INSERT, new BatchPreparedStatementSetter() {
                    ...
                    public void setValues(PreparedStatement preparedStatement, int i) throws SQLException {
                        try {
                            ...
                            int onlineMode = -1;
                            if (error != null) {
                                statusId = error.getPayloadStatus();
                                onlineMode = error.getOnlineMode(payload.getOfflineMode().intValue());
                            }
